...
class StanfordCoreNLP:
    def __init__(self, path_or_host, port=9000, memory='8g', lang='en', timeout=15000, quiet=True,
        ...
        if path_or_host.startswith('http'):
            ...
            '''while self.port in [conn.laddr[1] for conn in psutil.net_connections()]:
            ...
            #args = 'java -Xmx4g -cp \"*\" edu.stanford.nlp.pipeline.StanfordCoreNLPServer -port 9000 -timeout 15000'
            print(args)
            logging.info(args)
    ...
    def seg_dparse(self, sentence): #tokenize and dependency parser
        r_dict = self._request('ssplit,tokenize,depparse', sentence)
        return [token['word'] for s in r_dict['sentences'] for token in s['tokens']],[(dep['dep'], dep['governor'], dep['dependent']) for s in r_dict['sentences'] for dep in
				s['basicDependencies']]
...
if __name__=='__main__':
    ...
    sentence_list= ['this is an apple', 'this is a table']
